...
--miniconda miniconda_version="4.3.31" \
  conda_install="python=3.6 pytest  traits pandas matplotlib=2.1.2 scikit-learn seaborn nbformat" \
...
--user=neuro \
--run-bash 'source activate neuro && cd	/data && datalad install -r ///workshops/nih-2017/ds000114 && cd ds000114 && paths="sub-*/ses-test/anat  sub-*/ses-test/func/*fingerfootlips* derivatives/fmriprep/sub-*/anat/*space-mni152nlin2009casym_preproc.nii.gz  derivatives/fmriprep/sub-*/anat/*t1w_preproc.nii.gz  derivatives/fmriprep/sub-*/anat/*h5  derivatives/freesurfer/sub-01" && datalad --report-status=failure get -r -J4 "$paths"' \
